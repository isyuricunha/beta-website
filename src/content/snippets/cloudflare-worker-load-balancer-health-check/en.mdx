...
description: >-
  A free, practical load balancer using Cloudflare Workers + Cloudflare Tunnel,
...
This solves the common Cloudflare Tunnel limitation: you can’t bind the same hostname to multiple tunnels, but you _can_ bind different hostnames and put a Worker in front.
...
Problem: Cloudflare will route jellyfin.domain.com to ANY server (A, B, C or D randomly), but Jellyfin is only on Server A! 💥
Result:
