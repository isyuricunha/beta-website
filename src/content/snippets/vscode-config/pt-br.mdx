...
description: >-
  Configuração completa do VSCode e VSCodium com lista de extensões e backup de
  settings para migrar facilmente entre instalações.
tags:
  - vscode
  - vscodium
  - configuration
  ...
  - extensions
locale: pt-br
publishedDate: 2025-12-05T00:00:00.000Z
category: general
---
...
<Callout type='warning'>
  Estas são **configurações pessoais**. Revise e ajuste antes de aplicar no seu ambiente. Algumas
  opções como caminhos de arquivos, chaves de API e preferências podem precisar de personalização.
</Callout>
...
<Callout type='warning'>
  O arquivo `settings.json` contém uma chave de API (`gitCommitSuggestions.openAIKey`). Remova ou
  substitua por uma chave sua, ou melhor ainda, use variáveis de ambiente para dados sensíveis.
</Callout>

<Callout type='info'>
  Antes de aplicar em uma instalação nova, revise cada configuração para garantir que bate com seu
  fluxo de trabalho. Algumas opções, como caminhos de arquivo, podem precisar de ajuste para o seu
  ambiente.
</Callout>
