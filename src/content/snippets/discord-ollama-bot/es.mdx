---
title: Bot de Discord con Ollama y SQLite
description: >-
  Un bot asíncrono completo de Discord que se integra con una instancia local de
  Ollama y realiza un seguimiento del historial de usuarios en SQLite.
tags:
  - python
  - discord
  - ollama
...
Este fragmento proporciona una implementación robusta y basada en clases de un bot de Discord que conversa utilizando un LLM local a través de Ollama. Incluye procesamiento asíncrono de mensajes, límites de concurrencia (semáforos), respuestas en streaming y seguimiento persistente del historial de usuarios mediante SQLite.

{/* more */}

## Características

- **Asíncrono y Concurrent**: Utiliza colas y semáforos de `asyncio` para manejar múltiples canales de manera eficiente sin sobrecargar el LLM local.
- **Respuestas en Streaming**: Transmite tokens desde Ollama a Discord en tiempo real (o en fragmentos) para una experiencia más interactiva.
- **Conciencia del Contexto**: Mantiene el historial de conversación por usuario/canal, recortado a un límite configurable.
- **Persistencia**: Registra la actividad del usuario (última vez visto, nombre de usuario) en una base de datos local SQLite.
- **Arquitectura Limpia**: Refactorizado en clases (`Config`, `Database`, `OllamaClient`, `Bot`) siguiendo los principios SOLID.

## El Código

Guarda este archivo como `discord_ollama_bot.py`.
...
1. **Instalar Dependencias**:
   ...
   ```
2. **Ollama**: Asegúrate de que Ollama esté en ejecución (`ollama serve`) y de que hayas descargado el modelo (por ejemplo, `ollama pull llama3`).
3. **Token**: Reemplaza `DISCORD_TOKEN` en la clase `Config` con el token real de tu bot del Portal para Desarrolladores de Discord.

<Callout type='warning'>
  Nunca subas tu token real de Discord a un repositorio público. Usa variables de entorno (por
  ejemplo, `python-dotenv`) para implementaciones en producción.
</Callout>
