---
title: OllamaとSQLiteを使ったDiscordボット
description: ローカルのOllamaインスタンスと統合し、SQLiteでユーザー履歴を追跡する非同期Discordボットの完全ガイド。
...
- **クリーンアーキテクチャ**: `Config`、`Database`、`OllamaClient`、`Bot`クラスにリファクタリングされ、SOLID原則に従っています。
...
実際のDiscordトークンをパブリックリポジトリにコミットしないでください。本番環境では環境変数（例:
`python-dotenv`）を使用してください。
</blockquote>
