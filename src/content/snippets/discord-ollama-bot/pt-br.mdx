...
description: >-
  Um bot completo e assíncrono do Discord que integra com uma instância local do
  Ollama e registra histórico de usuários no SQLite.
...
- **Assíncrono e concorrente**: usa filas do `asyncio` e semáforos para lidar com múltiplos canais de forma eficiente, sem sobrecarregar o LLM local.
- **Respostas em streaming**: faz streaming de tokens do Ollama para o Discord em tempo real, ou em blocos, para dar sensação de resposta rápida.
...
- **Arquitetura limpa**: organizado em classes (`Config`, `Database`, `OllamaClient`, `Bot`) seguindo princípios SOLID.
...
2. **Ollama**: garanta que o Ollama está rodando (`ollama serve`) e que você já puxou o modelo (ex.: `ollama pull llama3`).
...
Nunca faça commit do seu token real do Discord em um repositório público. Em produção, use
variáveis de ambiente (por exemplo, `python-dotenv`).
</blockquote>
