---
title: Discord-Bot mit Ollama & SQLite
description: >-
  Ein vollständiger, asynchroner Discord-Bot, der mit einer lokalen
  Ollama-Instanz integriert ist und den Nutzerverlauf in SQLite speichert.
...
Dieses Snippet bietet eine robuste, klassbasierte Implementierung eines Discord-Bots, der über ein lokales LLM mit Ollama kommuniziert. Es umfasst asynchrone Nachrichtenverarbeitung, Nebenläufigkeitsbegrenzungen (Semaphore), Streaming-Antworten und persistente Benutzerverlaufsverfolgung mit SQLite.
## Funktionen

- **Asynchron & Nebenläufig**: Nutzt `asyncio`-Warteschlangen und Semaphore, um mehrere Kanäle effizient zu verarbeiten, ohne das lokale LLM zu überlasten.
- **Streaming-Antworten**: Streamt Tokens von Ollama in Echtzeit (oder in Blöcken) an Discord für ein interaktives Gefühl.
- **Kontextbewusstsein**: Behält den Gesprächsverlauf pro Benutzer/Kanal bei und kürzt ihn auf eine konfigurierbare Grenze.
- **Persistenz**: Protokolliert Benutzeraktivitäten (zuletzt gesehen, Benutzername) in einer lokalen SQLite-Datenbank.
- **Saubere Architektur**: Refaktorisiert in Klassen (`Config`, `Database`, `OllamaClient`, `Bot`) nach SOLID-Prinzipien.

## Der Code

Speichere diese Datei als `discord_ollama_bot.py`.
...
1. **Abhängigkeiten installieren**:
   ...
   ```
2. **Ollama**: Stelle sicher, dass Ollama ausgeführt wird (`ollama serve`) und dass du das Modell heruntergeladen hast (z. B. `ollama pull llama3`).
3. **Token**: Ersetze `DISCORD_TOKEN` in der `Config`-Klasse mit deinem tatsächlichen Bot-Token aus dem Discord Developer Portal.

<blockquote data-callout="warning">
Gib dein tatsächliches Discord-Token niemals in ein öffentliches Repository ein. Verwende für
Produktivumgebungen Umgebungsvariablen (z. B. `python-dotenv`).
</blockquote>
