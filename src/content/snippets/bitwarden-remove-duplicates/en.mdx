...
<Callout type='warning'>
  Always backup your vault before doing any cleanup operations. Export your current vault before
  importing the deduplicated one, just to be safe.
</Callout>

<Callout type='info'>
  If you want to check for duplicates based on different fields (like just username + URI), you can
  modify the `key` tuple in the code to include only the fields you care about.
</Callout>
